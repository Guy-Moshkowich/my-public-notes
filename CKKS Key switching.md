...
$\begin{array}{r}a\cdot s\_{in}+m+e \\ -a( a^\*s\_{out}+s\_{in}+e^\*)\\ \hline -aa^\*s\_{out} + m+e-ae^\* \end{array}$ 
...
\\
\hline -P^{-1}aa^\*s\_{out} + m+ \underbrace{e-P^{-1}ae^\* +e\_{\text{round}}}\_{\text{small error}}\bmod Q \\
\end{array}$$ 
